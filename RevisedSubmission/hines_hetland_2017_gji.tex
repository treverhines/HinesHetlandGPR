...
parameters in $C_{u_i}$, $\mitbf{\theta} = \{\ell, \phi, \tau \}$.
Just as in Section \ref{sec:NoiseModel}, we pick $\mitbf{\theta}$ to
maximize the probability of drawing $\data_i^*$ from $\data_i$, but
now $\data_i$ includes $u_i(\points)$, and $\mitbf{\theta}$ refers to
the parameters for $C_{u_i}(\points,\points)$ rather than
$\mitbf{C}_{\eta_i}$. We divide the GNSS data into seven subsets that
are four months long and each centered on the time of a SSE. The times
of the seven SSEs are determined with tremor records from
\cite{Wech2010}. We find the optimal $\mitbf{\theta}$ for each subset
of data, for each displacement component, and for each choice of
$T_i$. We choose to make each data subsets four months long because it
is long enough to encompass a SSE in the Pacific Northwest, while it
is still computationally tractable to estimate $\mitbf{\theta}$. The
REML likelihood function that we are maximizing with respect to
$\mitbf{\theta}$ is now
...
$\hat{\data}_i$ for the winter 2015-2016 SSE at stations ALBH, P436,
and SC02. Based on Figure \ref{fig:Fit}, the chosen covariance
function for $T_i$ has almost no effect on $\hat{\data}_i$. The
posterior displacements for the IBM covariance function contain
...
shown for $\hat{\data}_\e$ when $T_i$ is an IBM or Wendland covariance
function, but the uncertainties are nearly equivalent to the
uncertainties for when $T_i$ is the SE covariance function. The
posterior displacements for the different covariance functions are all

...
C_{u}\left((x,t),(x',t')\right) = \alpha^2 X(x,x')T(t,t'),
\end{equation}  
where $\alpha$ is a hyperparameter. The appropriate choice for $X$ and $T$ may vary depending on the geophysical signal we are trying to describe (e.g. postseismic deformation or deformation from slow slip events), and we discuss this matter in the next section.  
...
X(x,x') = \exp\left(\frac{-||x - x'||_2^2}{2 \beta^2}\right).
\end{equation}
...
T(t,t') &= \int_0^t \int_0^{t'} \min(\tau,\tau') \,d\tau'\,d\tau \\
        &= \frac{1}{2}\min(t,t')^2 \left(\max(t,t') - \frac{1}{3}\min(t,t')\right), \ \ \ t,t' \geq 0.
\end{align}
...
T(t,t') = \left(1 - \frac{|t - t'|}{\beta}\right)^3_+ \left(\frac{3|t - t'|}{\beta} + 1\right), 
\end{equation}
where
\begin{equation}
(t)_+ = 

...
In this study, we use GNSS data which has been made publicly available by the University Navstar Consortium (UNAVCO) \citep{Herring2016} and can be found at www.unavco.org.  We limit the dataset to the stations and time ranges which are pertinent to seven of the most recent SSEs in the Puget Sound region; the earliest SSE began in the August 2010, and the most recent SSE began in the Winter of 2017. We consider these most recent SSEs because the station coverage is sufficiently dense for us to make well constrained inferences of optimal prior models.  The distribution of stations considered in this station is shown in Figure X. 


\begin{figure*}
\includegraphics{figures/outliers/outliers.pdf}
\caption{Detrended easting component of GNSS displacement data for station SC03, which is located on Mt. Olympus in Washington.  The red markers indicate outliers which were automatically detected using the algorithm from Section \ref{sec:Method}. The error bars show one standard deviation uncertainty.}   
\label{fig:1}
\end{figure*}
...
We next want to determine appropriate hyperparameters for $X$ and each of the three candidate functions for $T$. We use the REML method to find the optimal hyperparameters for describing each of the seven SSEs. Before using the REML method, we clean the GNSS datasets by removing offsets at times of equipment changes and removing outliers with the method describe in Section X. For the outlier detection algorithm, our prior model, $u$, is chosen to be have a length-scale and time-scale which is sufficiently short to describe SSE displacements. We use the Squared Exponential covariance function for $X$ with $\beta = 100$ km, and we use the Wendland covariance function for $T$, due to its computational efficiency, with $\beta = 0.1$ yr$^{-1}$.  The outlier detection algorithm is particularly effective at removing outliers for stations at high altitudes, which can be adversely affected by ice or snow \citep{Lisowski2008}. As an example, Figure X shows the outliers which were automatically detected for station SC03, which is located on Mount Olympus in Washington. Note that most of the detected outliers occur during the winter and can likely be attributed to ice or snow.  After cleaning the dataset, we divide it into four month long subsets, where each subset is roughly centered on the time of an SSE.  We identify the times of seven SSEs using tremor records from \cite{Wech2010}. For each data subset, we use the REML method to find the hyperparameters for $T$ and $X$ that best describe deformation from the SSE. Our motivation analyzing subsets of the data is two-fold: (1) it keeps the analysis computationally tractable and (2) the distribution of estimated hyperparameters will indicates whether we can use a single covariance model to describe all SSEs. Our hyperparameter estimates are summarized in Table X. 

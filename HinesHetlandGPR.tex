...
The prior for each component of transient displacements is then conditioned with $\bm{d}_*$ to form a posterior estimate of transient displacements, $\hat{u} = u | \bm{d}_*$. We will assume that an appropriate covariance function and corresponding hyperparameters for $X$, $T$, and $C_\eta$ have already been chosen. If $\kappa$ is kept finite, then following \citet{Rasmussen2006} we find that $\hat{u}$ is distributed as $\mathcal{N}(\mu_{\hat{u}},C_{\hat{u}})$, where

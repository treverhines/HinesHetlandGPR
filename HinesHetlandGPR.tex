...
In our formulation for estimating transient strain rates, we have assumed that noise in the data vector is normally distributed. This is not an appropriate assumption for GNSS displacement time series which are prone to more outliers than would be predicted for normally distributed noise. It follows that proposed methods for analyzing GNSS timeseries should be robust against outliers \citep[e.g.,][]{Blewitt2016}. In order to make our estimates of transient strain more robust, we automatically identify and remove outliers in the GNSS data as a pre-processing step. Our strategy for identifying outliers is based on the data editing algorithms described in \citet{Gibbs2011}. In essence, outliers are considered to be observations which are abnormally inconsistent with $\bm{d}$. This algorithm is particularly effective because $\bm{d}$ has a spatial and temporal dependence, which allows us to identify anomalous observations which may not be apparent by inspection of timeseries alone.          

We now describe the data editing algorithm. Let $\Omega$ be the index set identifying non-outliers in $\bm{d}_*$,  which initially contains all $n$ indices.  

We again treat $\bm{d}_*$ as a sample of $\bm{d}$ and we assume some prior covariance function for $u$. We let $\epsilon$ be uncorrelated noise which has standard deviation given by the formal data uncertainties, $\bm{\sigma}$. Outliers are considered to be the observations which are abnormally inconsistent with the Gaussian process $u(\bm{y}) + \bm{P}\bm{a}$. We identify outliers in an iterative algorithm that is based on the data editing algorithm described in \citet{Gibbs2011}, which we now describe.  

Let $\Omega$ be the index set of non-outliers in $\bm{d}_*$, which we initiate with all $n$ indices. For each iteration we calculate the residual vector
\begin{align}
...
as an approximate realization of $\epsilon$.



in addition to spatially and/or temporally correlated noise.

  be Furthermore, we consider $\epsilon$ to be composed of uncorrelated noise with standard deviations given by the formal uncertainties on $\bm{d}_*$, $\bm{\sigma}$, as well as  as well as s composed of     

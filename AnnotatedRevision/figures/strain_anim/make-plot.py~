...
bm = Basemap(projection='tmerc',
             ...
             llcrnrlon = -125.7,llcrnrlat = 45.2,
             urcrnrlon = -119.6,urcrnrlat = 49.8)
...
key_pos_display = ax.transAxes.transform((0.715,0.205))
...
mjd_start = pygeons.mjd.mjd(start_date,'%Y-%m-%d')
mjd_stop = pygeons.mjd.mjd(stop_date,'%Y-%m-%d')

def plot_init():
  # clear any existing strain glyphs
  while(len(glyphs) > 0):
    g = glyphs.pop()
    g.remove()  
...
def plot_date(mjd_date):
  ...
  stamp = pygeons.mjd.mjd_inv(mjd_date,'%Y-%m-%d')
  print(stamp)
  ax.set_title(stamp,fontsize=10)

ani = FuncAnimation(fig,plot_date,frames=np.arange(mjd_start,mjd_stop),init_func=plot_init)
ani.save('strain-animation.mp4',writer='ffmpeg',dpi=250)

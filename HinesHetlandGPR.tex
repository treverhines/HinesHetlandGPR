...
In this paper we have demonstrated that Gaussian process regression (GPR) is a powerful tool for estimating transient strain rates from GNSS data. Other methods for estimating strain from GNSS rely on parametric representation of deformation, which can bias the estimated strain rates if the parameterization is not chosen carefully. In contrast, the GPR method is non-parametric and by the suchoice Our estimated transient strain rates have meaningful uncertainties, which distinguishes it from other methods of estimating strain from GNSS data.  As a result, we are able to confidently distinguish geophysical signal from noise. The key advantage that GPR has over other methods for estimating strain rate, is that GPR   

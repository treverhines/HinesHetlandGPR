...
\end{equation}  
As long as the functions $X_i$ and $T_i$ are positive definite, $C_{u_i}$ is guaranteed to also be positive definite and thus a valid covariance function \citep[sec. 4.2.4]{Rasmussen2006}. In order to compute transient strain rates from transient displacements, it is also necessary that the derivatives $\partial^2 X_i(\vec{x},\vec{x}')/ \partial x_j \partial x_j'$ and $\partial^2 T_i(t,t') / \partial t \partial t'$ exist everywhere. The appropriate choice for $X_i$ and $T_i$ may vary depending on the geophysical signal we are trying to describe, and we discuss this matter in Section \ref{sec:SignalModel}. 
...
We now have a posterior estimate of transient displacements that can be evaluated at any $p$. We can write the components of $\dot\varepsilon$ as 
...
\end{equation}
The components of $\dot\varepsilon$ are stochastic processes, and we can consider realizations of $\dot{\varepsilon}_{ij}$ to be realizations of $\hat{u}_i$ and $\hat{u}_j$ that have undergone the operations from eq. (\ref{eq:StrainRate}). Since the operations from eq. (\ref{eq:StrainRate}) are linear, we know that $\dot\varepsilon_{ij}$ is also a Gaussian process. From \citet[sec. 10.2]{Papoulis1991}, we find that $\dot{\varepsilon}_{ij}$ has a mean function

...
For $X_i$, we use the squared exponential (SE) covariance function,
...
parameters in $X_i$ and $T_i$, $\mitbf{\theta} = \{\ell, \phi, \tau
\}$. we divide the GNSS data into seven subsets that are four months
long and each centered on the time of a SSE. The times of the seven
SSEs are determined with tremor records from \cite{Wech2010}. We find
the optimal $\mitbf{\theta}$ for each subset of data, for each
displacement component, and for each choice of $T_i$. We choose to
make each data subsets four months long because it is long enough to
encompass a SSE in the Pacific Northwest, while it is still
computationally tractable to estimate $\mitbf{\theta}$. However, four
months is too short to resolve the sinusoids in $\data_i$, and they
are omitted from $\data_i$ in this REML analysis. The REML likelihood
function that we are maximizing with respect to $\mitbf{\theta}$ is
now 
...
the median of estimated parameters should be an appropriate prior
model for all Pacific Northwest SSEs. For the IBM model, there are
several anomalously large values for $\ell$ and $\phi$, which is why
the interquartile ranges are large.

\begin{table}\label{tab:Parameters}
\caption{
Optimal parameters for the prior on transient displacements determined
with the REML method. The temporal covariance function is indicated by
the ``$T$'' column. The SE, IBM, and Wendland covariance functions are
defined in eqs. (\ref{eq:TimeSE}), (\ref{eq:IBM}), and
(\ref{eq:Wendland}), respectively. The spatial covariance function,
$X$, is the squared exponential (eq. \ref{eq:SE}) in all cases. The
parameters are estimated for each of the seven SSEs considered in
...
functions for $T$ and the median parameters from Table 1. Figure
...
that the chosen time-scale parameter, $\tau$, is too large. The
...
use the median values from Table 1 for the parameters. We choose

...
A posterior estimate of transient displacement is formed by conditioning $\vec{u}$ with GNSS observations. GNSS data records transient displacements as well as other physical and non-physical processes that we are not interested in. We describe GNSS observations at position $\vec{x}_i$ and time $t_j$ as a realization of the random variable 
\begin{align}\label{eq:Data}
\begin{split}
\vec{d}_{ij} = &\vec{u}(\vec{x}_i,t_j) + \vec{\eta}(\vec{x}_i,t_j) + \vec{w}_{ij} + \vec{a}^{(1)}_i + \vec{a}^{(2)}_it_j + \\
               &\vec{a}^{(3)}_i\sin(2 \pi t_j) + \vec{a}^{(4)}_i\cos(2 \pi t_j) + \vec{a}^{(5)}_i\sin(4 \pi t_j) + \vec{a}^{(6)}_i\cos(4 \pi t_j), 
\end{split}
\end{align}
where $\vec{a}^{(1)}_{i}$ is an offset that is unique for each GNSS station, $\vec{a}^{(2)}_{i}$ is the secular velocity at $\vec{x}_i$, and the sinusoids describe seasonal deformation (using units of years for $t_j$). We use $\vec{w}_{ij}$ to denote normally distributed, uncorrelated noise. Correlated noise which does not have a parametric representation is denoted by $\vec{\eta}$.  For example, $\vec{\eta}$ can consist of temporally correlated noise describing benchmark wobble \citep[e.g.,][]{Wyatt1982,Wyatt1989}, and/or spatially correlated noise describing common mode error \citep[e.g.,][]{Wdowinski1997}. For now, we will assume that each component of $\vec{\eta}$ is a Gaussian process distributed as $\eta_i \sim \mathcal{N}(0,C_{\eta_i})$, and $C_{\eta_i}$ is a spatially and temporally dependent covariance function. We consider the coefficients in eq. (\ref{eq:Data}) to be uncorrelated random variables distributed as $\mathcal{N}(0,\kappa^2)$ in the limit as $\kappa \to \infty$ (i.e., the coefficients have diffuse priors). Of course, the secular velocities, $\vec{a}^{(2)}_{i}$, are spatially correlated and we could invoke a tectonic model to form a prior on $\vec{a}^{(2)}_{i}$. However, in our application to the Pacific Northwest, we will be using displacement time series which are long enough to sufficiently constrain $\vec{a}^{(2)}_{i}$ for each station, avoiding the need to incorporate a prior. Likewise, seasonal deformation is spatially correlated \citep{Dong2002,Langbein2008}, and it may be worth exploring and exploiting such a correlation in a future study. We ignore any covariances between the easting and northing components of $\vec{d}_{ij}$. This simplifies our analysis by allowing us to constrain each component of transient displacements independently.    

Each component of the posterior transient displacements, denoted as $\hat{u}_i$, is constrained by conditioning $u_i$ with GNSS observations of displacements along direction $i$, which we write as $\mitbf{d}^*_i$. The data vector consists of $n$ observations, which have been made at $m$ stations. We let $\mitbf{P}$ be the set of $(\vec{x}_i, t_j)$ pairs describing where and when each of the observations have been made. We also let $\mitbf{a}_i$ be the length $6m$ vector of coefficients from eq. (\ref{eq:Data}) for each station and for direction $i$. We use $\mitbf{G}$ to represent the $n \times 6m$ matrix of corresponding basis functions evaluated at each point in $\mitbf{P}$. We can then view $\mitbf{d}^*_i$ as a realization of the random vector
\begin{equation}

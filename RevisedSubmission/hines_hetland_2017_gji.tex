...
We provide an example to give a more tangible meaning to the
covariance function for $u_i$. we can use a squared exponential
function for $X_i$ and $T_i$,
...
displacements. The covariance function for $u_i$ and one realizations
for $u_i$ are shown in Figure \ref{fig:PriorDemo} using the parameters
$\ell = 100$ km, $\tau = 10$ days, and $\phi = 1$ mm. While the
...
to describe the spatial and temporal covariance (eq.

...
\begin{summary}
Transient strain rates derived from GNSS data can be used to detect and understand geophysical phenomena such as slow slip events and postseismic deformation. Here, we propose using Gaussian process regression (GPR) as a tool for estimating transient strain rates from GNSS data. GPR is a non-parametric, Bayesian method for interpolating scattered data. Transient strain rates estimated with GPR have meaningful uncertainties, which allow us to easily discern geophysical signal from noise. In our approach, we assume a stochastic prior model for transient displacements. The prior describes how much we expect transient displacements to covary spatially and temporally. A posterior estimate of transient strain rates is obtained by differentiating the prior after conditioning it with GNSS data. We apply our method to GNSS data from Cascadia, where our intention is to detect transient strain resulting from slow slip events. We use maximum likelihood methods to constrain a prior model describing surface displacements resulting from slow slip events. The temporal covariance of our prior model is described by a compact Wendland covariance function, which significantly reduces the computational burden that is often associated with GPR. Our results reveal the spatial and temporal evolution of strain from slow slip events. We verify that the transient strain estimated with GPR is in fact geophysical signal by comparing it to the seismic tremor that is associated with slow slip events in Cascadia.
\end{summary}
...
Crustal strain rates are fundamentally important quantities for assessing seismic hazard. Knowing where and how quickly strain is accumulating gives insight into where we can expect stored elastic energy to be released seismically. Consequently, secular crustal strain rates estimated from GNSS data have been used to constrain seismic hazard models such as UCERF3 \citep{Field2014}. Transient crustal strain, which we consider to be any deviation from secular strain rates, is also relevant for assessing seismic hazard. Transient strain resulting from geophysical phenomena such as slow slip events or postseismic deformation is itself innocuous and can only be detected with high precision geodetic methods. However, there are several instances of major earthquakes being triggered by these transient strain events \citep{Roeloffs2006,Freed2001}. Dense networks of continuous GNSS stations, such as the Plate Boundary Observatory (PBO), make it possible to identify transient strain with high fidelity. An active area of research is on developing and improving upon methods for deriving strain and, specifically transient strain, from GNSS displacement observations. 
